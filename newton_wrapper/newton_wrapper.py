...
from .utils import convert_to_timestamp, response_to_json
...
class Newton:
    ...
    def __generate_signature_date(self, method, path, content_type="", body=""):
        ...
        # If the request has a body, you would use this instead of empty
        # string below (replace BODY with actual request body):
        if body != "":
        ...
        computed_signature = hmac.new(
            self.__secret_key.encode(ENCODING), msg=signature_data, digestmod=sha256
        ).digest()
    ...
    def get_actions(
        ...
        NewtonAPIAuth, NewtonDate = self.__generate_signature_date("GET", "/actions")
    ...
    def get_balances(self, asset=""):
        NewtonAPIAuth, NewtonDate = self.__generate_signature_date("GET", "/balances")
        headers = {"NewtonAPIAuth": NewtonAPIAuth, "NewtonDate": NewtonDate}

        params = {"asset": asset}
        r = requests.get(BASE_URL + "/balances", headers=headers, params=params)
        return response_to_json(r.text)
    ...
    def get_order_history(
        ...
        offset="",
        symbol="",
        time_in_force: TimeInForce = TimeInForce.NONE,
    ):
        NewtonAPIAuth, NewtonDate = self.__generate_signature_date("GET", "/order/history")
        headers = {"NewtonAPIAuth": NewtonAPIAuth, "NewtonDate": NewtonDate}
        ...
        r = requests.get(BASE_URL + "/order/history", headers=headers, params=params)
        return response_to_json(r.text)

    def get_open_orders(
        self, limit="", offset="", symbol="", time_in_force: TimeInForce = TimeInForce.NONE
    ):
        NewtonAPIAuth, NewtonDate = self.__generate_signature_date("GET", "/order/open")
        headers = {"NewtonAPIAuth": NewtonAPIAuth, "NewtonDate": NewtonDate}
        ...
        r = requests.get(BASE_URL + "/order/open", headers=headers, params=params)
        return response_to_json(r.text)

    # order_type = ["LIMIT"]
    # side = ["BUY", "SELL"]
    # Open order: {'order_type': ['This field is required.'],
    # 'time_in_force': ['This field is required.'],
    ...
    def cancel_order(self, order_id):
        ...
        r = requests.post(BASE_URL + "/order/cancel", headers=headers, data=body)
        return response_to_json(r.text)
